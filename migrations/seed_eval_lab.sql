-- ============================================================
-- Eval Lab seed — Clerk-compatible, idempotent.
-- Run once in the Supabase SQL editor. Safe to re-run.
--
-- Replaces db/seed-eval-lab.sql (which predates the Clerk migration:
-- it used `user_id uuid references users(id)` — prod's users.id is now
-- text — and old auth.uid() RLS policies). Access is via the service-role
-- client, which bypasses RLS, so we enable RLS with NO policies (locked to
-- service role) exactly like migrations/enable_rls.sql.
-- ============================================================

create table if not exists eval_tickets (
...
-- vibe_check_sessions already exists in prod (Clerk migration re-added its FK);
-- guarded here for fresh databases. user_id is TEXT (Clerk id), not uuid.
create table if not exists vibe_check_sessions (
...
create index if not exists idx_ticket_labels_session on ticket_labels(session_id, slot_number);
create index if not exists idx_vibe_sessions_user on vibe_check_sessions(user_id, started_at desc);

-- Defense in depth: RLS on, no policies (service role bypasses; anon is locked out).
...
-- ============================================================
-- 20-TICKET DATASET (idempotent upsert)
-- ============================================================
...
on conflict (slot_number) do update set
  ...
  agent_urgency = excluded.agent_urgency,
  agent_reasoning = excluded.agent_reasoning,
  agent_escalate = excluded.agent_escalate,
