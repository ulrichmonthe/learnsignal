...
import { HARNESS_COURSE, HE_ARTIFACT_PREVIEWS } from '@/lib/courses/harness-engineering'

export default function HarnessEngineeringPage() {
  const totalLessons = HARNESS_COURSE.modules.reduce((sum, m) => sum + m.lessons.length, 0)
  const completedSlugs = useCourseProgress('harness-engineering')
  const orderedSlugs = HARNESS_COURSE.modules.flatMap(m => m.lessons.map(l => l.slug))
    ...
    <div className="min-h-[calc(100vh-57px)]">
      <div className="max-w-[780px] mx-auto px-6 pt-14 pb-24">
        ...
        <div className="mb-12">
          ...
          >
            {HARNESS_COURSE.description}
          </p>

          <div className="mt-6">
            <CourseCTA hrefBase="/playground/learn/harness-engineering" orderedSlugs={orderedSlugs} completedSlugs={completedSlugs} />
          </div>
        </div>

        {/* Modules */}
        <div className="space-y-10">
          {HARNESS_COURSE.modules.map((module, mIdx) => (
            <div key={module.slug}>
              {/* Module header */}
              ...
              <div className="space-y-2">
                {module.lessons.map(lesson => (
                  <Link
                    key={lesson.slug}
                    href={`/playground/learn/harness-engineering/${lesson.slug}`}
        ...
        >
          ...
          >
            Ten lessons. Five artifacts. One production-grade harness for an autonomous coding agent.
          </p>
          <div className="space-y-2">
            {HE_ARTIFACT_PREVIEWS.map((artifact, i) => (
        ...
        >
          ...
          >
            Practice the harness in the PCE Lab
          </h3>
          <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.5)', fontFamily: 'var(--font-dm-sans)', lineHeight: '1.6', marginBottom: '14px' }}>
            Practice the harness decisions in the PCE Lab — 10 missions, real test tickets, live scoring →
          </p>
          <Link
