...
import {
  ...
  HELesson9Body,
  HELesson10Body,
} from '@/components/courses/harness-engineering-bodies'
...
const LESSON_BODIES: Record<string, React.ComponentType> = {
  'lesson-1': HELesson1Body,
  'lesson-2': HELesson2Body,
  'lesson-3': HELesson3Body,
  'lesson-4': HELesson4Body,
  'lesson-5': HELesson5Body,
  'lesson-6': HELesson6Body,
  'lesson-7': HELesson7Body,
  'lesson-8': HELesson8Body,
  'lesson-9': HELesson9Body,
  'lesson-10': HELesson10Body,
}

export default async function HELessonPage({ params }: PageProps) {
  const { lesson: lessonSlug } = await params
  const lesson = getHELessonBySlug(lessonSlug)

  if (!lesson) notFound()

  const { prev, next } = getAdjacentHELessons(lessonSlug)
      ...
      <div className="flex max-w-[1200px] mx-auto px-4 pt-10 pb-24 gap-0">
        ...
        <main style={{ flex: 1, minWidth: 0, maxWidth: '680px' }}>
          ...
          >
            {prev ? (
              <Link
                href={`/playground/learn/harness-engineering/${prev.slug}`}
                className="group"
            ...
            {next ? (
              <Link
                href={`/playground/learn/harness-engineering/${next.slug}`}
        ...
        <aside
          ...
          style={{ width: '240px' }}
        >
          <ArtifactPanel artifacts={[]} previewArtifacts={HE_ARTIFACT_PREVIEWS} />
        </aside>
      </div>
    </div>
  )
