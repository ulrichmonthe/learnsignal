...
import { AGENT_ORCHESTRATION_COURSE, AO_ARTIFACT_PREVIEWS } from '@/lib/courses/agent-orchestration'

export default function AgentOrchestrationPage() {
  const totalLessons = AGENT_ORCHESTRATION_COURSE.modules.reduce((sum, m) => sum + m.lessons.length, 0)
  const completedSlugs = useCourseProgress('agent-orchestration')
  const orderedSlugs = AGENT_ORCHESTRATION_COURSE.modules.flatMap(m => m.lessons.map(l => l.slug))
    ...
    <div className="min-h-[calc(100vh-57px)]">
      <div className="max-w-[780px] mx-auto px-6 pt-14 pb-24">
        ...
        <div className="mb-12">
          ...
          >
            Track 02 · Course · {totalLessons} Lessons · {AGENT_ORCHESTRATION_COURSE.estimatedMinutes} Minutes
          </p>
          <h1
            className="font-display font-medium text-text leading-tight mb-4"
            style={{ fontSize: 'clamp(26px, 4vw, 36px)', fontStyle: 'italic' }}
          >
            {AGENT_ORCHESTRATION_COURSE.title}
          </h1>
          <p
            style={{
              fontSize: '16px',
          ...
          >
            {AGENT_ORCHESTRATION_COURSE.description}
          </p>

          <div className="mt-6">
            <CourseCTA hrefBase="/playground/learn/agent-orchestration" orderedSlugs={orderedSlugs} completedSlugs={completedSlugs} />
          </div>
        </div>

        {/* Modules */}
        <div className="space-y-10">
          {AGENT_ORCHESTRATION_COURSE.modules.map((module, mIdx) => (
            <div key={module.slug}>
              ...
              <div className="space-y-2">
                {module.lessons.map(lesson => (
                  <Link
                    key={lesson.slug}
                    href={`/playground/learn/agent-orchestration/${lesson.slug}`}
        ...
        >
          ...
          >
            Eleven lessons. Five scenarios where you make the call before the technique is named. And a review you can run at work on Monday.
          </p>
          <div className="space-y-2">
            {AO_ARTIFACT_PREVIEWS.map((artifact, i) => (
