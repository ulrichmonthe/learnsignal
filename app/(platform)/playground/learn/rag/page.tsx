...
import { RAG_COURSE, RAG_ARTIFACT_PREVIEWS } from '@/lib/courses/rag'

export default function RAGCoursePage() {
  const totalLessons = RAG_COURSE.modules.reduce((sum, m) => sum + m.lessons.length, 0)
  const completedSlugs = useCourseProgress('rag')
  const orderedSlugs = RAG_COURSE.modules.flatMap(m => m.lessons.map(l => l.slug))
    ...
    <div className="min-h-[calc(100vh-57px)]">
      <div className="max-w-[780px] mx-auto px-6 pt-14 pb-24">
        ...
        <div className="mb-12">
          ...
          >
            Course · {totalLessons} Lessons · {RAG_COURSE.estimatedMinutes} Minutes
          </p>
          <h1
          ...
          >
            {RAG_COURSE.description}
          </p>

          <div className="mt-6">
            <CourseCTA hrefBase="/playground/learn/rag" orderedSlugs={orderedSlugs} completedSlugs={completedSlugs} />
          </div>
        </div>

        {/* Modules */}
        <div className="space-y-10">
          {RAG_COURSE.modules.map((module, mIdx) => (
            <div key={module.slug}>
              {/* Module header */}
              ...
              <div className="space-y-2">
                {module.lessons.map(lesson => (
                  <Link
                    key={lesson.slug}
                    href={`/playground/learn/rag/${lesson.slug}`}
        ...
        >
          ...
          >
            Sixteen lessons. Five artifacts. One production-grade retrieval system for a doc-grounded AI product.
          </p>
          <div className="space-y-2">
            {RAG_ARTIFACT_PREVIEWS.map((artifact, i) => (
        ...
        >
          ...
          >
            Build Helix in the RAG Lab
          </h3>
          <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.5)', fontFamily: 'var(--font-dm-sans)', lineHeight: '1.6', marginBottom: '14px' }}>
            Put every concept from this course into practice across graded missions. Diagnose a broken retrieval
            pipeline, tune chunking and reranking, and isolate generation failures from retrieval failures →
          </p>
          <Link
            href="/playground/rag-lab"
            className="inline-block font-mono rounded px-4 py-2 transition-opacity hover:opacity-90"
            style={{
              fontSize: '10px',
